...
function mapper(collection, subfolder, type, fileExt, value) {
    return collection.map(function(title, i) {
        return new Item({ name: namer(title, type, i, collection.length),
            image: subfolder + "/" + title + fileExt, value: value });
    });
}

function namer(base, type, count, total) {
    var title = ion.titleCase(base.replace(/_/g," "));
    return ion.format("{|}{0} {1} collectible (#{2} of {3})", title, type, (count+1), total);
}

var rn = ["I","II","III","IV","V","VI","VI","VII","VIII","IX","X","XI","XII","XIII","XIV","XV","XVI","XVII","XVIII","XIX","XX","XXI","XXII","XXIII","XIV"];
var encyclopedias = [];

ion.times(24, function(count) {
    var letter = (count === 23) ? "XYZ" : String.fromCharCode(count + 65);
    var name = "{|}Encyclopedia Britannica, vol. " + rn[count] + " (letter "+letter+", collectible #"+(count+1)+" of 24)";
    encyclopedias.push(new Item({
        name: name,
        value: 10
    }));
});
...
var collectibles = {
    movies: mapper(movieTitles, "movies", "movie poster", ".jpg", 10),
    propaganda: mapper(warTitles, "war", "propaganda poster", ".gif", 10),
